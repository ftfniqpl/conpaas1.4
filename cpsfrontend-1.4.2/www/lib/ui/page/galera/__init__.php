...
*/	}
	...
	private function renderCommand() {
        ...
        //$master_ip, $master_port = $master_addr['ip'].split(":");
        $_master = split(":",$master_addr['ip']);
		$cmd = 'mysql --host='.$_master[0].' --port='.$_master[1].' -u '.$users[0].' -p';
	...
	public function renderAccessForm() {
		...
		'<div class="form-section mysql-access">'
			.'<div class="form-header">'
				...
				.'<div class="clear"></div>'
			.'</div>'
			.$this->renderFormRow('server address', $this->service->getAccessLocation())
			.$this->renderUsers()
			.$this->renderFormRow('<img src="images/terminal.png" title="shell command" />', $this->renderCommand())
